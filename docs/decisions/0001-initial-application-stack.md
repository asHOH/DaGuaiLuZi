...
The application is a friends-only, turn-based card game for mobile and desktop browsers. It runs as one application instance on an existing VPS through `cloudflared`. The server must own game state, protect hidden information during active Hands, persist completed-hand history, support read-only Hand Replay with independent viewer playback positions and playable Challenge Hands through shareable Challenge Codes, and support persistent username/password accounts with optional email. Exact recovery of an in-progress room after a restart is optional.

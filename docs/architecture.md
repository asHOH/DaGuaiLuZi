...
The Hand Seed and original deals remain server-private until the Hand ends. Completed-Hand history then exposes the original deals and a reusable Challenge Code, but not the Hand Seed. A Hand Replay is read-only, single-viewer playback of the source history. The Challenge Code may also initialize a separate playable Challenge Hand whose six players receive the same logical-seat setup and produce independent actions and results.
